...
class DetailPokeApiActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        setContentView(R.layout.activity_detail_poke_api)
    ...
    private fun searchById(query: String) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val myResponse: Response<PokeApiDetailResponse> =
                    retrofit.create(ApiService::class.java).getPokemonDetail(query)

                if (myResponse.isSuccessful) {
                    val response: PokeApiDetailResponse? = myResponse.body()
